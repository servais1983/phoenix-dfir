...
API_KEY_GOOGLE = "VOTRE_API_KEY_ICI"
...
# --- FONCTION D'EXTRACTION D'INFORMATION ---
def extraire_et_sauvegarder_conclusions(analyse_text, filename):
    global dossier_enquete
    print("\n--- [PHOENIX-CORRELATION] Extraction des informations clés du rapport...")
        ...
        for ioc_type, ioc_list in extracted_data.items():
            if ioc_type in dossier_enquete.get("iocs", {}) and isinstance(ioc_list, list):
                for ioc_value in ioc_list:
                    if ioc_value not in dossier_enquete["iocs"][ioc_type]:
                        dossier_enquete["iocs"][ioc_type].append(ioc_value)
        print("--- [PHOENIX-CORRELATION] Dossier d'enquête mis à jour avec succès. ---")
...
def main():
    global dossier_enquete
    print("="*50)
    print("Bienvenue dans PHOENIX v2.1 - L'Enquêteur Autonome")
    print("Commandes: nouvelle_enquete, resume_enquete, ajoute_ioc, analyse, quitter")
    print("="*50)
    ...
    while True:
        ...
        elif command == "ajoute_ioc":
            if not dossier_enquete: print("\n--- [PHOENIX] Veuillez d'abord créer une enquête avec 'nouvelle_enquete'. ---")
            elif len(parts) == 3:
                ioc_type = parts[1].lower()
                ioc_value = parts[2]
                if ioc_type in dossier_enquete.get('iocs', {}):
                    if ioc_value not in dossier_enquete['iocs'][ioc_type]:
                        dossier_enquete['iocs'][ioc_type].append(ioc_value)
                        print(f"--- [PHOENIX] IoC ajouté au dossier : [{ioc_type}] {ioc_value} ---")
                    else: print(f"--- [PHOENIX] Info : Cet IoC est déjà dans le dossier. ---")
                else: print(f"\nPhoenix Aide: Type d'IoC non valide. Types valides : ips, hashes, domaines")
            else: print("\nPhoenix Aide: Format -> ajoute_ioc <type> <valeur>")
